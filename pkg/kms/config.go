...
func (k *CryptoKeyConfig) ToString() string {
	...
	if k.validated || k.parsed || k.AutoGenerated {
...
func (k *CryptoKeyConfig) loadEnvVar() error {
	...
	if v == "" {
		return fmt.Errorf("environment variable %s not found", k.EnvVarName)
	}
	k.EnvVarValue = v
	return nil
}
...
func ParseCryptoKeyConfigs(cfg string, log *zap.Logger) ([]*CryptoKeyConfig, error) {
		...
		*/
		...
		for i < max {
			...
			if exists := reservedKeyConfigKeywords[args[i]]; exists && (remainder == 0) {
				return nil, fmt.Errorf("key config entry %q is invalid: keyword %q must not be last", line, args[i])
			}
			...
			case "token":
				if remainder != 2 {
					return nil, fmt.Errorf("key config entry %q is invalid: token must be followed by its attributes", line)
				}
				switch args[i+1] {
				case "name":
				...
				case "lifetime":
					...
					if err != nil {
						return nil, fmt.Errorf("key config entry %q contains invalid token lifetime: %v, error: %v", line, args[i+2], err)
					}
					key.TokenLifetime = i
				default:
					return nil, fmt.Errorf("key config entry %q contains invalid token property: %v", line, args[i+1])
				}
				i += 2
			case "verify", "sign", "both", "auto":
				if key.Usage != "" {
					return nil, fmt.Errorf("key config entry %q contains duplicate key id", line)
				}
				key.Usage = args[i]
				if args[i+1] != "from" {
					if remainder > 1 {
						return nil, fmt.Errorf("key config entry %q is invalid: bad syntax", line)
					}
					if key.Source != "" {
						return nil, errors.ErrCryptoKeyConfigMultipleDefaultKeys
					}
					key.Secret = args[i+1]
					key.Source = "config"
					key.Algorithm = "hmac"
				...
				case 3:
					...
					case "env":
						...
						if err := key.loadEnvVar(); err != nil {
							return nil, fmt.Errorf("key config entry %q is invalid: %v", line, err)
						}
					default:
						return nil, fmt.Errorf("key config entry %q is invalid: bad syntax", line)
					}
			...
			default:
				return nil, fmt.Errorf("key config entry %q contains invalid argument: %s", line, args[i])
			}
			i++
		}
	}
	...
	for _, cfg := range keys {
		...
		if err := cfg.validate(); err != nil {
			return nil, fmt.Errorf("key config %d is invalid: %v", cfg.Seq, err)
		}
		cfg.parsed = true
	}
	return keys, nil
